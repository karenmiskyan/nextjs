...
const Paypal = ({customToken, setIsLoading, address, address2, address3, check, tax}) => {
    ...
    const order = () => {
        ...
        fetch(`${APICallUrl}/api/checkout-process`, {
            ...
            if (res.error === false) {
                dispatch(setPaypalToken(res?.data?.paypal_payment_id))
                dispatch(setMyToken(customToken))
                router.push(res?.data.checkoutUrl, undefined, {target: '_blank'});
                setIsLoading(false);
            } else {
                setErr(res.message)
                setIsLoading(false);
        ...
        <Row className="g-3">
            ...
            <Col lg="12">
                <div className='product-buttons' style={{flexDirection: "column"}}>
                    <Button style={{backgroundColor: "#ffc439", width: "100%", border: "none"}}
